...
import { addBlogPost, editBlogPost } from '../../redux/actions/blogPosts';

const BlogPostForm = ({ blogPostId, setBlogPostId }) =>{
    ...
    const post = useSelector((state)=>
        blogPostId 
            ? state.post.find((message) => message._id === blogPostId)
            : null
    )
    ...
    const handleSubmit = async (e)=>{
        ...
        if(blogPostId === 0){
            dispatch(addBlogPost(blogInfo));
        }else{
            dispatch(editBlogPost(blogInfo));
        }
    }
